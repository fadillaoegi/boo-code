...
test('tracker merangkum progres, tool, todo, file, keamanan, review, dan verifikasi', () => {
  ...
  tracker.record({ type: 'change-impact', changedFiles: 1, affectedFiles: 8, tests: 2, edges: 11, maxDepth: 3, blastRadius: 'medium', truncated: false })
  tracker.record({ type: 'lsp-session', stage: 'started', openDocuments: 1 })
  tracker.record({ type: 'lsp-session', stage: 'reused', openDocuments: 2 })
  ...
  tracker.record({ type: 'context-trimmed', droppedMessages: 4, estimatedTokens: 900, prioritizedMessages: 2, dependencyMessages: 3, dependencyEdges: 2 })
  ...
  tracker.record({ type: 'tool-recovery', name: 'bash', callId: 'timeout', kind: 'timeout', reason: 'idle', category: 'test', durationMs: 1_000, idleTimeoutMs: 1_000, maximumTimeoutMs: 5_000, nextIdleTimeoutMs: 2_000, partialOutput: true })
  tracker.record({ type: 'verification-repair', stage: 'needed', round: 1, maxRounds: 3, revision: 1 })
  tracker.record({ type: 'verification-repair', stage: 'repaired', round: 1, maxRounds: 3, revision: 2 })
  ...
  assert.equal(state.contextDependencyMessages, 3)
  assert.equal(state.contextDependencyEdges, 2)
  ...
  assert.equal(state.toolTimeoutRecoveries, 1)
  assert.equal(state.verificationRepairRounds, 1)
  assert.equal(state.verificationRepairs, 1)
  assert.equal(state.changeImpactAnalyses, 1)
  assert.equal(state.changeImpactAffectedFiles, 8)
  assert.equal(state.changeImpactEdges, 11)
  assert.equal(state.changeImpactDepth, 3)
  assert.equal(state.changeImpactBlastRadius, 'medium')
  assert.equal(state.lspSessionStarts, 1)
  assert.equal(state.lspSessionReuses, 1)
  assert.equal(state.lspOpenDocuments, 2)
  assert.match(formatTaskStatus(state), /1 argumen invalid.*Todo: 1\/2 selesai.*Verifikasi.*File terdampak.*src\/app\.ts.*Tool protocol: 1 peringatan · 1 fallback model.*Evidence cache: 1 hit.*Context relevance: 2 pesan.*Context dependency: 3 pesan melalui 2 relasi.*Parallel discovery: 3 tool call.*Tool result store: 1 hasil besar.*Timeout recovery: 1 command.*Verification repair: 1 putaran · 1 pulih.*Change impact: 8 file · 11 relasi · depth 3 · blast radius medium.*LSP session: 1 baru · 1 reuse · 0 restart · 2 dokumen aktif.*prompt injection/s)
})
